...
namespace SIDCOP.IntegrationTest.Controllers
{
    ...
    public class RolesIntegrationTest : BaseIntegrationTest
    {
        ...
        [TestMethod]
        public async Task Roles_ListarPantallas()
        {
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            var response = await cliente.GetAsync("/Roles/ListarPantallas");
            Assert.AreEqual(System.Net.HttpStatusCode.OK, response.StatusCode);
            Assert.IsNotNull(response);
            var responseContent = await response.Content.ReadAsStringAsync();
            Assert.IsFalse(string.IsNullOrEmpty(responseContent));
        ...
        [TestMethod]
        public async Task Roles_ListarAccionesPorPantalla()
        {
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            var response = await cliente.GetAsync("/Roles/ListarAccionesPorPantalla");
            Assert.AreEqual(System.Net.HttpStatusCode.OK, response.StatusCode);
            Assert.IsNotNull(response);
            var responseContent = await response.Content.ReadAsStringAsync();
            Assert.IsFalse(string.IsNullOrEmpty(responseContent));
        ...
        public async Task Roles_Insertar()
        {
        ...
        public async Task Roles_Actualizar()
        {
            // PASO 1: CONFIGURACIÓN DEL CLIENTE HTTP
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            ...
            var rolesMock = RolesMocks.CrearMockRolActualizar();
            ...
            var response = await cliente.PutAsync("/Roles/Actualizar", contenido);
        ...
        [TestMethod]
        public async Task Roles_Buscar()
        {
            // PASO 1: CONFIGURACIÓN DEL CLIENTE HTTP
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            // Usa un método diferente del mock para crear datos de búsqueda
            var rolesMock = RolesMocks.CrearMockRolBuscar();
            // PASO 3: SERIALIZACIÓN 
            var contenido = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(rolesMock),
                System.Text.Encoding.UTF8,
            ...
            var response = await cliente.GetAsync($"/Roles/Buscar/{rolesMock.Role_Id}");
            // VERIFICACIONES 
            Assert.AreEqual(System.Net.HttpStatusCode.OK, response.StatusCode);
            Assert.IsNotNull(response);
            var responseContent = await response.Content.ReadAsStringAsync();
        ...
        [TestMethod]
        public async Task Roles_Eliminar()
        {
            // PASO 1: CONFIGURACIÓN DEL CLIENTE HTTP
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            // Usa un método diferente del mock para crear datos de eliminación
            var rolesMock = RolesMocks.CrearMockRolEliminar();
            // PASO 3: SERIALIZACIÓN 
            var contenido = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(rolesMock),
                System.Text.Encoding.UTF8,
            ...
            //  PETICIÓN A ENDPOINT DE ELIMINACIÓN
            var response = await cliente.PostAsync($"/Roles/Eliminar/{rolesMock.Role_Id}", contenido);
            // VERIFICACIONES 
            Assert.AreEqual(System.Net.HttpStatusCode.OK, response.StatusCode);
            Assert.IsNotNull(response);
            var responseContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task Roles_Insertar_DeberiaFallar()
        {
            // PASO 1: CONFIGURACIÓN DEL CLIENTE HTTP
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("X-API-Key", ApiKey);
            ...
            var rolesExtremos = RolesMocks.CrearMockRolValoresExtremos();

            // PASO 3: SERIALIZACIÓN DE DATOS EXTREMOS
            var contenido = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(rolesExtremos),
                System.Text.Encoding.UTF8,
                "application/json"
            );
