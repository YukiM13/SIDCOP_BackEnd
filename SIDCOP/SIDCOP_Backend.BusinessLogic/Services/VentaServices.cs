...
namespace SIDCOP_Backend.BusinessLogic.Services
{
    public class VentaServices
    {
        ...
        public IEnumerable<tbVendedores> ListarVendedoresDDL()
        {
            try
            {
                var list = _vendedorRepository.ListDDL();
                return list;
            }
            catch (Exception ex)
            {
        ...
        public ServiceResult ListarResumenFiltrado(int vendId)
        {
            var result = new ServiceResult();
            try
            {
                var response = _cuentasporcobrarRepository.ListarResumenFiltrado(vendId);
